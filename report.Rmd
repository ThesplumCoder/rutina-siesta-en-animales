...
```{r}
dreaming_sample <- subset(animals_data, !(is.na(dreaming)))
...
for (iter in 1:length(dreaming_sample$species)) {
  specie <- dreaming_sample[iter, ]
  residual <- dreaming_estimator(specie$predation, specie$exposure, specie$danger) - dreaming_proportion[iter]
